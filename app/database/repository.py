...
def get_config(user_id: str, key: str, default: str | None = None) -> str | None:
    res = get_client().table("user_config").select("value").eq("user_id", user_id).eq("key", key).execute()
    return res.data[0]["value"] if res.data else default


def set_config(user_id: str, key: str, value: str) -> None:
    get_client().table("user_config").upsert({"user_id": user_id, "key": key, "value": str(value)}, on_conflict="user_id,key").execute()


def already_processed(user_id: str, chat_id: int, message_id: int) -> bool:
    return bool(get_client().table("processed_messages").select("id").eq("telegram_chat_id", chat_id).eq("telegram_message_id", message_id).execute().data)


def mark_processed(user_id: str, chat_id: int, message_id: int) -> None:
    try:
        get_client().table("processed_messages").insert({"user_id": user_id, "telegram_chat_id": chat_id, "telegram_message_id": message_id}).execute()
    except Exception:
        pass
...
def log_activity(user_id: str, date: str, platform: str, activity_type: str, quantity: int = 1, account_name: str | None = None, notes: str | None = None) -> dict:
    res = get_client().table("daily_activity").insert({"user_id": user_id, "date": date, "platform": platform, "account_name": account_name, "activity_type": activity_type, "quantity": quantity, "notes": notes}).execute()
    return res.data[0]
...
def log_content(user_id: str, platform: str, content_type: str, **kwargs) -> dict:
    ...
    res = get_client().table("content").insert(payload).execute()
    return res.data[0]
...
def update_content_metrics(content_id: str, **metrics) -> dict:
    res = get_client().table("content").update(metrics).eq("id", content_id).execute()
...
def get_booked_sales_calls_for_date(user_id: str, booked_date: str) -> list[dict]:
    return get_client().table("sales_calls").select("*").eq("user_id", user_id).eq("booked_date", booked_date).eq("outcome", "Booked").order("booked_time").execute().data
...
def get_pending_followups(user_id: str, as_of_date: str | None = None) -> list[dict]:
    ...
    if as_of_date:
        q = q.lte("follow_up_date", as_of_date)
...
def create_meeting(user_id: str, title: str, start_time_iso: str, person: str | None = None, meeting_type: str | None = None, end_time_iso: str | None = None, notes: str | None = None) -> dict:
    res = get_client().table("meetings").insert({"user_id": user_id, "title": title, "person": person, "meeting_type": meeting_type, "start_time": start_time_iso, "end_time": end_time_iso, "notes": notes, "status": "scheduled"}).execute()
    return res.data[0]
...
def find_meeting_by_person_or_title(user_id: str, search_text: str, only_upcoming: bool = True) -> list[dict]:
    ...
    if only_upcoming:
        q = q.gte("start_time", _utc_now_iso())
    res = q.order("start_time").execute()
    text = search_text.lower().strip()
    return [m for m in res.data if text in (m.get("person") or "").lower() or text in (m.get("title") or "").lower()]


def update_meeting(meeting_id: str, **fields) -> dict:
    fields["updated_at"] = _utc_now_iso()
    res = get_client().table("meetings").update(fields).eq("id", meeting_id).execute()
    return res.data[0] if res.data else {}
...
def get_meeting(meeting_id: str) -> dict | None:
    res = get_client().table("meetings").select("*").eq("id", meeting_id).execute()
...
def get_next_meeting(user_id: str) -> dict | None:
    res = get_client().table("meetings").select("*").eq("user_id", user_id).neq("status", "cancelled").gte("start_time", _utc_now_iso()).order("start_time").limit(1).execute()
    return res.data[0] if res.data else None
...
def create_reminder(user_id: str, reminder_text: str, trigger_time_iso: str, related_meeting_id: str | None = None, recurrence: str | None = None) -> dict:
    res = get_client().table("reminders").insert({"user_id": user_id, "reminder_text": reminder_text, "trigger_time": trigger_time_iso, "related_meeting_id": related_meeting_id, "recurrence": recurrence, "status": "pending"}).execute()
    return res.data[0]


def cancel_reminders_for_meeting(meeting_id: str) -> None:
    get_client().table("reminders").update({"status": "cancelled"}).eq("related_meeting_id", meeting_id).eq("status", "pending").execute()


def get_due_reminders(as_of_iso: str) -> list[dict]:
    return get_client().table("reminders").select("*").eq("status", "pending").lte("trigger_time", as_of_iso).execute().data


def mark_reminder_sent(reminder_id: str) -> None:
    get_client().table("reminders").update({"status": "sent", "sent_at": _utc_now_iso()}).eq("id", reminder_id).execute()


def cancel_reminder(reminder_id: str) -> None:
    get_client().table("reminders").update({"status": "cancelled"}).eq("id", reminder_id).execute()


def update_reminder(reminder_id: str, **fields) -> dict:
    res = get_client().table("reminders").update(fields).eq("id", reminder_id).execute()
    return res.data[0] if res.data else {}


def find_pending_reminder(user_id: str, search_text: str) -> list[dict]:
    res = get_client().table("reminders").select("*").eq("user_id", user_id).eq("status", "pending").order("trigger_time").execute()
    text = search_text.lower().strip()
...
def find_booked_sales_call(user_id: str, lead_name: str, booked_date: str | None = None, phone_number: str | None = None) -> dict | None:
    q = get_client().table("sales_calls").select("*").eq("user_id", user_id).eq("outcome", "Booked")
    if lead_name:
        q = q.ilike("lead_name", lead_name)
    if booked_date:
        q = q.eq("booked_date", booked_date)
    if phone_number:
        q = q.eq("phone_number", phone_number)
    rows = q.order("booked_date").order("booked_time").execute().data
...
def cancel_booked_sales_call_and_reminders(user_id: str, lead_name: str, booked_date: str | None = None, phone_number: str | None = None) -> int:
    db = get_client()
    q = db.table("sales_calls").select("id,lead_name,booked_date,booked_time,phone_number").eq("user_id", user_id).eq("lead_name", lead_name).eq("outcome", "Booked")
    if booked_date:
        q = q.eq("booked_date", booked_date)
    if phone_number:
        q = q.eq("phone_number", phone_number)
    ...
    db.table("sales_calls").update({"outcome": "Cancelled"}).in_("id", ids).execute()
    meetings = db.table("meetings").select("id,person,start_time").eq("user_id", user_id).eq("status", "scheduled").execute().data
    for call in rows:
        for meeting in meetings:
            if (meeting.get("person") or "").strip().lower() != (call.get("lead_name") or "").strip().lower():
                continue
            start = str(meeting.get("start_time") or "")
            if call.get("booked_date") and call["booked_date"] not in start:
                continue
            if call.get("booked_time") and str(call["booked_time"])[:5] not in start:
                continue
...
def get_daily_digest_data(user_id: str, target_date: str, created_after: str | None = None) -> dict:
    return {
        "work": get_activity(user_id, target_date, target_date, created_after=created_after),
        "content": get_content(user_id, target_date, target_date, created_after=created_after),
        "sales": get_sales_calls(user_id, target_date, target_date, created_after=created_after),
        "meetings": get_meetings_in_range(user_id, f"{target_date}T00:00:00", f"{target_date}T23:59:59", created_after=created_after),
        "tasks": get_assigned_tasks_for_date(user_id, target_date, created_after=created_after),
    }
