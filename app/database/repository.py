...
def get_or_create_user(telegram_user_id: int, name: Optional[str] = None) -> dict:
    db=get_client(); res=db.table("users").select("*").eq("telegram_user_id",telegram_user_id).execute()
    if res.data: return res.data[0]
    res=db.table("users").insert({"telegram_user_id":telegram_user_id,"name":name or settings.USER_NAME,"timezone":settings.DEFAULT_TIMEZONE}).execute(); return res.data[0]
...
def get_activity(user_id:str,start_date:str,end_date:str)->list[dict]: return get_client().table("daily_activity").select("*").eq("user_id",user_id).gte("date",start_date).lte("date",end_date).execute().data

def log_content(user_id:str,platform:str,content_type:str,**kwargs)->dict:
    payload={"user_id":user_id,"platform":platform,"content_type":content_type,"post_date":kwargs.get("post_date") or dt.date.today().isoformat(),**{k:v for k,v in kwargs.items() if k!="post_date"}}; res=get_client().table("content").insert(payload).execute(); return res.data[0]

def get_content(user_id:str,start_date:str,end_date:str)->list[dict]: return get_client().table("content").select("*").eq("user_id",user_id).gte("post_date",start_date).lte("post_date",end_date).execute().data
...
def get_sales_calls(user_id:str,start_date:str,end_date:str)->list[dict]: return get_client().table("sales_calls").select("*").eq("user_id",user_id).gte("date",start_date).lte("date",end_date).execute().data
...
def get_pending_followups(user_id:str,as_of_date:str|None=None)->list[dict]:
    q=get_client().table("sales_calls").select("*").eq("user_id",user_id).not_.is_("follow_up_date","null").in_("outcome",["Follow Up","Interested","Proposal","Rescheduled"]); return q.order("follow_up_date").execute().data
...
def find_meeting_by_person_or_title(user_id:str,search_text:str,only_upcoming:bool=True)->list[dict]:
    ...
    if only_upcoming:q=q.gte("start_time",dt.datetime.utcnow().isoformat())
...
def update_meeting(meeting_id:str,**fields)->dict:
    fields["updated_at"]=dt.datetime.utcnow().isoformat(); res=get_client().table("meetings").update(fields).eq("id",meeting_id).execute(); return res.data[0] if res.data else {}
...
def get_meetings_in_range(user_id:str,start_iso:str,end_iso:str)->list[dict]: return get_client().table("meetings").select("*").eq("user_id",user_id).neq("status","cancelled").gte("start_time",start_iso).lte("start_time",end_iso).order("start_time").execute().data
...
def mark_reminder_sent(reminder_id:str)->None:get_client().table("reminders").update({"status":"sent","sent_at":dt.datetime.utcnow().isoformat()}).eq("id",reminder_id).execute()
...
def find_booked_sales_call(user_id:str,lead_name:str,booked_date:str|None=None,phone_number:str|None=None)->dict|None:
    ...
    if lead_name:q=q.ilike("lead_name",lead_name)
...
def cancel_booked_sales_call_and_reminders(user_id:str,lead_name:str,booked_date:str|None=None,phone_number:str|None=None)->int:
    db=get_client(); q=db.table("sales_calls").select("id").eq("user_id",user_id).eq("lead_name",lead_name).eq("outcome","Booked")
    if booked_date:q=q.eq("booked_date",booked_date)
    if phone_number:q=q.eq("phone_number",phone_number)
    rows=q.execute().data; ids=[r["id"] for r in rows]
    if not ids:return 0
    db.table("sales_calls").update({"outcome":"Cancelled"}).in_("id",ids).execute()
    db.table("reminders").update({"status":"cancelled"}).in_("related_meeting_id",ids).eq("status","pending").execute()
    return len(ids)
...
def get_daily_digest_data(user_id:str,target_date:str,created_after:str|None=None)->dict:return {"work":get_activity(user_id,target_date,target_date),"content":get_content(user_id,target_date,target_date),"sales":get_sales_calls(user_id,target_date,target_date),"meetings":get_meetings_in_range(user_id,f"{target_date}T00:00:00",f"{target_date}T23:59:59"),"tasks":get_assigned_tasks_for_date(user_id,target_date,created_after=created_after)}
