...
from app.bot.inline_actions import PENDING_CONFLICTS, conflict_keyboard

logger = logging.getLogger(__name__)

async def _reject_unauthorized(update: Update) -> bool:
    if not repo.is_authorized(update.effective_user.id): await update.message.reply_text("This is a private assistant. You're not authorized to use it."); return True
    return False

def _get_user_and_tz(update: Update):
    user = repo.get_or_create_user(update.effective_user.id, update.effective_user.first_name); return user, pytz.timezone(user.get("timezone") or settings.DEFAULT_TIMEZONE)

async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await _reject_unauthorized(update): return
    message = update.message; text = (message.text or "").strip()
    if not text: return
    user, tz = _get_user_and_tz(update)
    try:
        if repo.already_processed(user["id"], message.chat_id, message.message_id): return
    except Exception as exc: logger.warning("Duplicate check failed: %s", exc)
    try: await _dispatch(message, user, tz, parse_message(text), text)
    except Exception:
        logger.exception("Message handling failed"); await message.reply_text("Something went wrong while saving that. Nothing was recorded - please try again."); return
    try: repo.mark_processed(user["id"], message.chat_id, message.message_id)
    except Exception: pass

async def handle_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await _reject_unauthorized(update): return
    api_key = os.getenv("GROQ_API_KEY") or os.getenv("OPENAI_API_KEY")
    if not api_key: await update.message.reply_text("Voice transcription is not configured yet."); return
    user, tz = _get_user_and_tz(update); path = None
    try:
        with tempfile.NamedTemporaryFile(suffix=".ogg", delete=False) as tmp: path = tmp.name
        telegram_file = await context.bot.get_file(update.message.voice.file_id); await telegram_file.download_to_drive(path)
        url = "https://api.groq.com/openai/v1/audio/transcriptions" if os.getenv("GROQ_API_KEY") else "https://api.openai.com/v1/audio/transcriptions"
        model = os.getenv("WHISPER_MODEL", "whisper-large-v3-turbo" if os.getenv("GROQ_API_KEY") else "whisper-1")
        async with httpx.AsyncClient(timeout=90) as client:
            with open(path, "rb") as audio: response = await client.post(url, headers={"Authorization": f"Bearer {api_key}"}, data={"model": model, "language": "hi"}, files={"file": ("voice.ogg", audio, "audio/ogg")})
            response.raise_for_status(); text = response.json().get("text", "").strip()
        if not text: await update.message.reply_text("I couldn't hear the voice note clearly. Please try again."); return
        await update.message.reply_text(f"Heard: {text}"); await _dispatch(update.message, user, tz, parse_message(text), text)
    except Exception:
        logger.exception("Voice transcription failed"); await update.message.reply_text("I couldn't transcribe that voice note. Please try again.")
    finally:
        if path:
            try: os.unlink(path)
            except OSError: pass
...
async def handle_reschedule_shortcut(message, user, tz, text: str) -> bool:
    ...
    base = dt.datetime.now(tz) + dt.timedelta(days=days); new_start = tz.localize(dt.datetime.combine(base.date(), dt.time(hour, minute))); target = matches[0]
    ...
    await message.reply_text(f"Updated: {target['title']} → {new_start.strftime('%d %b at %I:%M %p').lstrip('0').replace(' 0', ' ')}."); return True
...
async def _handle_log_content(message, user, tz, parsed):
    ...
    for item in items:
        qty=int(item.get("quantity") or 1); platform=item.get("platform") or "Other"; content_type=item.get("content_type") or "Other"; account=item.get("account_name"); repo.log_activity(user["id"], today, platform, content_type.lower(), quantity=qty, account_name=account); saved.append(f"{qty} {content_type}{'s' if qty != 1 else ''} on {account or platform}")
...
async def _handle_log_sales(message, user, tz, parsed, raw_text):
    ...
    if sales.get("lead_name"):
        follow_up_date=None; booked_date=None
...
async def _handle_create_meeting(message, user, tz, parsed):
    ...
    if not start: await message.reply_text("What day/time should I schedule that for?"); return
    ...
    window_start=start-dt.timedelta(minutes=1); window_end=start+dt.timedelta(minutes=1)
    existing=repo.get_meetings_in_range(user["id"],window_start.isoformat(),window_end.isoformat())
    if existing:
        other=existing[0]
        PENDING_CONFLICTS[str(user["id"])]={"title":title,"person":person,"meeting_type":meeting_type,"start":start}
        await message.reply_text(f"⚠️ <b>Slot Conflict</b>\n{start.strftime('%d %b at %I:%M %p').lstrip('0').replace(' 0',' ')} is already booked with <b>{other.get('title') or other.get('person') or 'another meeting'}</b>.\n\nOverride or move to 4:30 PM?",parse_mode="HTML",reply_markup=conflict_keyboard()); return
    await _save_meeting(message,user,tz,meeting,start,meeting_type,is_sales_call,person,phone,title)

async def _save_meeting(message,user,tz,meeting,start,meeting_type,is_sales_call,person,phone,title):
    ...
    if is_sales_call:
        ...
        offset=int((meeting.get("reminder") or {}).get("offset_before_meeting_minutes") or settings.DEFAULT_MEETING_REMINDER_MINUTES); reminder_time=start-dt.timedelta(minutes=offset); reminder_text=f"{title} in {offset} minutes"; reminder_prefix="Reminder"
...
async def _handle_update_meeting(message,user,tz,parsed):
    ...
    if not search: await message.reply_text("Which meeting should I update?"); return
...
async def _handle_cancel_meeting(message,user,parsed):
    meeting=parsed.get("meeting") or {}; search=meeting.get("search_text") or meeting.get("person") or meeting.get("title")
    if not search: await message.reply_text("Which meeting should I cancel?"); return
    target=await _meeting_target(message,repo.find_meeting_by_person_or_title(user["id"],search))
    if not target:return
...
async def _handle_query_stats(message,user,tz,parsed):
    ...
    if metric=="sales_calls": rows=repo.get_sales_calls(user["id"],start_date.isoformat(),end_date.isoformat()); await message.reply_text(f"Sales calls: {len(rows)} ({start_date} to {end_date})")
    elif metric=="content": rows=repo.get_content(user["id"],start_date.isoformat(),end_date.isoformat()); await message.reply_text(f"Content entries: {len(rows)} ({start_date} to {end_date})")
    elif metric=="meetings": rows=repo.get_meetings_in_range(user["id"],dt.datetime.combine(start_date,dt.time.min,tzinfo=tz).isoformat(),dt.datetime.combine(end_date,dt.time.max,tzinfo=tz).isoformat()); await message.reply_text(f"Meetings: {len(rows)} ({start_date} to {end_date})")
    elif metric=="objections": rows=objections_list(user["id"],start_date,end_date); await message.reply_text(answer_objection_question(query.get("text") or "What objection patterns do you see?",[r.get("objection") or "Unknown" for r in rows]))
    else: await message.reply_text(_format_daily(daily_summary_data(user["id"],now.date()),now.date()),parse_mode="HTML")

async def _handle_general_question(message,parsed):
    query=parsed.get("query") or {}; question=query.get("text") or parsed.get("notes") or ""; await message.reply_text(answer_objection_question(question,[])) if question else await message.reply_text("I can log work, content, sales, meetings and reminders, or show your summaries. Try rephrasing that.")
