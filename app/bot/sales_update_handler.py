...
def _is_update_message(text: str) -> bool:
    low = text.lower()
    return bool(re.search(r"\b(update|change|edit)\b", low) or re.search(r"\b(?:ka|ki|ke)\s+(?:number|phone|call|status|type|time|date)\b", low)) and bool(_PHONE_RE.search(text) or re.search(r"\b(?:discovery|follow\s*up|demo|closing|strategy|completed|complete|interested|won|lost|time|date)\b", low) or _TIME_RE.search(text) or _DATE_RE.search(text))
...
async def _apply_update(message, target: dict, updates: dict) -> bool:
    ...
    if "booked_date" in updates or "booked_time" in updates:
        ...
        for meeting in meetings:
            ...
            new_date = dt.date.fromisoformat(updates.get("booked_date", current_ist.date().isoformat()))
            new_time = updates.get("booked_time", current_ist.strftime("%H:%M:%S"))
            ...
            if reminder_time > dt.datetime.now(ist):
                db.table("reminders").insert({"user_id": target["user_id"], "reminder_text": f"Confirm attendance: {target['lead_name']} — sales call at {naive.strftime('%I:%M %p').lstrip('0')}", "trigger_time": reminder_time.isoformat(), "related_meeting_id": meeting["id"], "recurrence": "none", "status": "pending"}).execute()
    ...
    lines = ["<b>✅  UPDATED SUCCESSFULLY</b>", "━━━━━━━━━━━━━━━━━━━━", f"• Lead: {_safe(target['lead_name'])}"]
    if "booked_date" in updates or "booked_time" in updates:
        date_value = updates.get("booked_date") or target.get("booked_date")
        time_value = updates.get("booked_time") or target.get("booked_time")
        try:
            d = dt.date.fromisoformat(str(date_value))
            t = dt.time.fromisoformat(str(time_value)[:8])
            lines.append(f"• Rescheduled To: {_safe(d.strftime('%A, %d %b'))} @ {_safe(dt.datetime.combine(d, t).strftime('%I:%M %p').lstrip('0'))}")
        except ValueError:
            pass
    if "phone_number" in updates:
        lines.append(f"• Phone: <code>{_safe(updates['phone_number'])}</code>")
    if "call_type" in updates:
        lines.append(f"• Call Type: {_safe(updates['call_type'])}")
    if "outcome" in updates:
        lines.append(f"• Status: {_safe(updates['outcome'])}")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
