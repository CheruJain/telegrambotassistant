...
def _looks_like_date_lookup(text: str) -> bool:
    value = text.lower()
    has_date = bool(
        re.search(r"\b(?:aaj|today|kal|tomorrow)\b", value)
        or re.search(r"\b\d{1,2}\s*(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\b", value)
    )
...
def _phone_for_meeting(meeting: dict, booked_calls: list[dict]) -> str | None:
    ...
    for call in booked_calls:
        ...
        if not lead or call.get("booked_date") != target_date:
            continue
        booked_time = str(call.get("booked_time") or "")[:5]
        if booked_time != target_time:
            continue
        if person and (person in lead or lead in person):
            matches.append(call)
        elif title and any(part and part in lead for part in re.split(r"\s+with\s+|,", title)):
            matches.append(call)
    phones = []
    for call in matches:
...
async def handle_date_details(message, user: dict, text: str) -> bool:
    ...
    reminders = repo.get_reminders_in_range(user["id"], start_local.isoformat(), end_local.isoformat())
    booked_calls = repo.get_booked_sales_calls_for_date(user["id"], target.isoformat())

    lines = [target.strftime("%d-%b-%Y"), ""]
    ...
    if meetings:
        for meeting in meetings:
            ...
            phone = _phone_for_meeting(meeting, booked_calls)
            if phone:
