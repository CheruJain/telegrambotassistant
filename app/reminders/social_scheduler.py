...
class SocialScheduler:
    def __init__(self,bot:Bot,chat_id:int,tz_name:str):self.bot=bot;self.chat_id=chat_id;self.tz=pytz.timezone(tz_name);self.scheduler=AsyncIOScheduler(timezone=self.tz)
    def start(self):
        ...
        self.scheduler.add_job(self._weekly,CronTrigger(day_of_week="sun",hour=11,minute=0,timezone=self.tz),id="social_weekly_followers",replace_existing=True);self.scheduler.start()
    ...
    async def _weekly(self):
        try:await self.bot.send_message(chat_id=self.chat_id,text=follower_prompt(self.chat_id),parse_mode="HTML")
